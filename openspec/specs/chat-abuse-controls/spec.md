...
Cost and abuse protection for the public chat endpoint, which spends a real API key: per-IP rate limiting, input/output caps, a per-session turn cap, and a global daily budget ceiling that degrades gracefully.
