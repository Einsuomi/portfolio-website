...
project has an on-site detail page SHALL be a single link to that page's `/projects/<slug>` route as its
primary affordance (the whole card clickable), navigating via a shared-element transition into the page;
the external repo link SHALL live on the detail page rather than on the card. A card whose project has no
detail page yet MAY link to its repo where one exists.
