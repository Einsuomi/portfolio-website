...
Centre BI. Each card SHALL carry the project name, a one-line summary, its tech stack, and a link to
its detail or repo where one exists.
...
The gallery SHALL scroll horizontally within its own bounds via touch, trackpad, and drag, and SHALL
...
The Projects gallery SHALL use the horizontal, image-forward treatment and SHALL NOT reuse the
vertical editorial layout of the Writes section, so the two sections read as deliberately different.
